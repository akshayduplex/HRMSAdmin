...
export default function RequisitionTableHod({ searchParamsInput, setSelectedRows }) {
    ...
    const { ManPowerRequisitionCardHod } = useSelector((state) => state.shortList)
    ...
    useEffect(() => {
        ...
        if (ManPowerRequisitionCardHod?.status === 'success') {
            setFirstLoading(false)
        }

    }, [ManPowerRequisitionCardHod?.status])

    useEffect(() => {
        const isPendingByHod = searchParams.get('type') === 'PendingByHod';
        let Payloads = {
            "employee_doc_id": userDetails?.employee_doc_id,
            "keyword": "",
            page_no: isPendingByHod ? 1 : paginationModel.page + 1,
            per_page_record: isPendingByHod ? 1000 : paginationModel.pageSize, // Fetch all data for table view
            "scope_fields": [],
            "status": "",
    ...
    useEffect(() => {
        if (ManPowerRequisitionCardHod.status === 'success') {
            let total = ManPowerRequisitionCardHod.data?.length
    ...
    }, [ManPowerRequisitionCardHod.status, ManPowerRequisitionCardHod.data, paginationModel.pageSize])
    ...
    const handleRedirectOnApprovalPage = (e, data) => {
        e.preventDefault();
        const CombineString = `${data._id}|${userDetails?.name}|HOD|${userDetails?.token}|${userDetails?.designation}|${userDetails?.employee_doc_id}`;
    ...
    const createDataGridRows = () => {
        if (!ManPowerRequisitionCardHod?.data) return [];

        return ManPowerRequisitionCardHod.data.map((item, index) => ({
            id: item._id || index,
            _id: item._id,
            title: item.title,
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Box sx={{
                    ...
                    <Tooltip title="View MPR" placement="top">
                        <MuiButton
                            variant="contained"
                            size="small"
                            color="success"
        ...
        },
        {
            ...
            // renderCell: (params) => moment(params.row.raised_on).format('DD/MM/YYYY') === 'Invalid Date' ? params.row.raised_on : moment(params.row.raised_on).format('DD/MM/YYYY')
        },
        {
    ...
    const isPendingByHodView = searchParams.get('type') === 'PendingByHod';

    return (
        <>
            {
                firstLoading && ManPowerRequisitionCardHod?.status === 'loading' ?
                    ...
                    isPendingByHodView ? (
                        // DataGrid Table View for PendingByHod
                        ...
                        <Row xs={1} md={2} lg={3} className="g-4 mt-0">
                            {
                                Array.isArray(ManPowerRequisitionCardHod?.data) && ManPowerRequisitionCardHod?.data?.length > 0 ? ManPowerRequisitionCardHod?.data?.map((key, idx) => {
