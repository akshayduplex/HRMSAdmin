...
const AppliedFromMaster = () => {
    ...
    const handleAddDivision = async (e) => {
        ...
        try {
            const response = await axios.post(`${config.API_URL}addAppliedFrom`, payload, apiHeaderToken(config.API_TOKEN));
    ...
    const handleGetDivisionList = async (input = '') => {
        ...
        try {
            const response = await axios.post(`${config.API_URL}getAppliedFromList`, payload, apiHeaderToken(config.API_TOKEN));
    ...
    const handleGetDivisionListTotal = async (input = '') => {
        ...
        try {
            const responseTotal = await axios.post(`${config.API_URL}getAppliedFromList`, payload, apiHeaderToken(config.API_TOKEN));
    ...
    const handleSubmitEdit = async (e) => {
        ...
        try {
            const response = await axios.post(`${config.API_URL}editAppliedFrom`, payload, apiHeaderToken(config.API_TOKEN));
            toast.success(response.data.message);
    ...
    const handleDelete = async (e , row) => {
        e.preventDefault();
        const payload = { _id: row?._id };
        if (!payload?._id) {
            return toast.warn('Applied From Id Is Required');
        }
        try {
            const response = await axios.post(`${config.API_URL}deleteAppliedFromById`, payload, apiHeaderToken(config.API_TOKEN));
            toast.success(response.data.message);
            setDivisionList(prevList =>
            ...
            renderCell: (params) => (
                <div style={{ display: "flex", gap: "10px" }}>
                    ...
                    <button
                        ...
                        onClick={(e) => handleDelete(e, params.row)} // 👈 Your delete handler
                        style={{ height: "35px", lineHeight: "12px" }}
                    >
                        <MdDeleteOutline className="fs-5 text-center" />
                    </button>
                </div>
            ),
        }
                                    ...
                                    <div className='row'>
                                        <div className="col-sm-6">
                                            <div className="mb-3">
                                                ...
                                                <Form.Control
                                                    type="text"
                                                    placeholder="Enter Applied From"
                                                    value={divisionName}
                                                    onChange={(e) => {
                                                        // const regex = /^[A-Za-z() ]+$/;
...
export default AppliedFromMaster;
