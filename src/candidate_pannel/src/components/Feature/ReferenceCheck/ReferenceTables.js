...
export default function ReferenceDataDataGrid({ ReferenceData, status, loading }) {
    ...
    const rows = useMemo(() => {
        return ReferenceData ? ReferenceData.map((record, index) => {
            return {
                id: index + 1,
    ...
    const columns = [
                                ...
                                <>
                                    ...
                                    <Tooltip
                                        ...
                                        componentsProps={{
                                            tooltip: {
                                                sx: {
                                                    backgroundColor: row.status === 'Rejected'
                                                        ? theme.palette.grey[500]
                                    ...
                                    >
                                        <span>
                                            <IconButton
                                                ...
                                                onClick={(e) => { } /* handleReject(e, row?.project) */}
                                                sx={{
                                                    transition: 'transform 0.3s',
                                                    '&:hover': row.status !== 'Rejected'
                                                        ? { transform: 'scale(1.2)', backgroundColor: 'rgba(244,67,54,0.1)' }
                                                        : {},
                                                }}
                                            >
                                                <Cancel
        ...
        { field: 'batch_id', headerName: 'batch ID', width: 100, flex: 1 },
        {
            field: 'employeeDetails',
            headerName: 'Name / Email / No. / Designations',
            width: 350,
            flex: 2,
            renderCell: (params) => {
        ...
        { field: 'applied_for', headerName: 'Job Title / Project Name', width: 200, type: 'number' },
        { field: 'posted_location', headerName: 'Job Location', width: 200, flex: 1 },
    ];
